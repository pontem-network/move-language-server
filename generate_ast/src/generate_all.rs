...
use crate::utils::{cargo_project_root, ensure_file_contents};
...
fn generate_ast() {
    ...
    let syntax_kinds_file = cargo_project_root().join("../parser/src/syntax_kind/generated.rs");
    let syntax_kinds = generate_syntax_kinds(KINDS_SRC);
    ensure_file_contents(syntax_kinds_file.as_path(), &syntax_kinds);

    let ast_tokens_file = cargo_project_root().join("../syntax/src/ast/tokens.rs");
    let contents = generate_tokens(&grammar_ast);
    ensure_file_contents(ast_tokens_file.as_path(), &contents);

    let ast_nodes_file = cargo_project_root().join("../syntax/src/ast/nodes.rs");
