...
fn set_rerun() {
    println!("cargo:rerun-if-env-changed=MOVE_LANGUAGE_SERVER_REV");
...
fn rev() -> String {
    if let Ok(rev) = env::var("MOVE_LANGUAGE_SERVER_REV") {
        return rev;
    }
    ...
    if let Some(commit_hash) = commit_hash() {
        ...
        let channel =
            env::var("MOVE_LANGUAGE_SERVER_CHANNEL").unwrap_or_else(|_| "dev".to_string());
        buf.push(' ');
